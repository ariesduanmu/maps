...
def parse_arguments():
    ...
    parser.add_argument('-w','--width', type=int, default=100, required=False, help="plot screen width")
    parser.add_argument('-e','--height', type=int, default=100, required=False, help="plot screen height")
...
def main():
    ...
    birds = Rooms(args.num_rooms, args.width, args.height, 1, 3)
    while True:
        print(birds.distance.sum())
        if birds.distance.sum() == 0:
            break
        birds.tick()

    board = [[0 for _ in range(args.width)] for _ in range(args.height)]
    for i in range(args.num_rooms):
        r, c = birds.centers[i]
        r = int(r)
        c = int(c)
        h, w = birds.sizes[i]
        if r-h >= 0 and r+h < args.width and c-w >= 0 and c+w < args.height:
            for x in range(r-h, r+h+1):
                for y in range(c-w, c+w+1):
                    board[x][y] = 2

    generate_image(board, (1000,1000))
