...
class Birds():
    def __init__(self, n, width, height):
        ...
        # width/height = 5-10
        self.size = ([15, 15] + 5 * np.random.rand(2*n).reshape(n, 2))*2
        self.minDistanceMatrix = squareform(pdist(self.size, lambda u, v: sum(np.hypot([u[0],v[0]],[u[1],v[1]]))/2))
        # pprint(self.minDistanceMatrix)
        angles = 2*math.pi*np.random.rand(n)
        self.vel = np.array(list(zip(np.sin(angles), np.cos(angles))))
        # self.vel = np.zeros(2*n).reshape(n, 2)
        self.n = n
    ...
    def applyRules(self):
        D = self.distMatrix < self.minDistanceMatrix
        pprint((D.sum(axis=1)).sum(axis=0))
